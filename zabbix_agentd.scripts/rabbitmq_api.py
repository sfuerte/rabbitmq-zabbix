...
Forked from https://github.com/jasonmcintosh/rabbitmq-zabbix

https://github.com/sfuerte/zbx-rabbitmq
'''
import json
import optparse
import socket
...
class RabbitMQAPI(object):
    ...
    def __init__(self, user_name='guest', password='guest', host_name='',
                 port=15672, conf='/etc/zabbix/zabbix_agentd.conf', senderhostname=None, protocol='http', disable_cert_check=False):
    ...
    def call_api(self, path):
        ...
        pass_handler = urllib2.HTTPBasicAuthHandler(password_mgr)

        ctx = ssl.create_default_context()

        if self.protocol == 'https' and self.disable_cert_check:
...
def main():
    ...
    parser.add_option('--disable-cert-check', help='Disable SSL cert verification for https',
                      default=False, action="store_true", dest="disable_cert_check",)
    ...
    parser.add_option('--logfile', help='File to log errors (defaults to /var/log/zabbix/rabbitmq_zabbix.log)',
                      default='/var/log/zabbix/rabbitmq_zabbix.log')
    ...
    api = RabbitMQAPI(user_name=options.username, password=options.password,
                      ...
                      protocol=options.protocol, disable_cert_check=options.disable_cert_check)
    if options.filters:
        try:
